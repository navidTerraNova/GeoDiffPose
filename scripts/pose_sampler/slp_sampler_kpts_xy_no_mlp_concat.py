...
class Identity(nn.Module):
    ...
    def forward(self, x):
        # 1. Check if channels (26) are at the end. If so, move them to dim 1.
        if x.ndim == 4 and x.shape[3] == 26:
            x = x.permute(0, 3, 1, 2)  
        # 2. Check if input is flat [Batch, 26]. If so, reshape.
        elif x.ndim == 2:
...
class SLPPoseBase(Dataset):
    ...
    def get_raw_keypoints(self, txt_path):
        ...
        all_pairs = []
        # Parse logic: assumes text file is still (x, y, v) or similar triplet structure
        ...
        # Filter redundant head points (indices 1,2,3,4)
        if len(all_pairs) >= 5:
...
def save_pose_visualization(pose_tensor, out_path, height, width):
    ...
    Assumes visibility is always true since we don't have 'v' channel.
    """
    data = pose_tensor.cpu().numpy().flatten()
    canvas = np.zeros((height, width, 3), dtype=np.uint8)
    
    points = []
    # Loop by 2 because data is [x, y, x, y...]
    ...
    for p1, p2 in limbs:
        if p1 < len(points) and p2 < len(points):
            pt_a = points[p1]
            pt_b = points[p2]
            # Draw line (We assume visible since we don't have v)
...
def get_parser():
    parser = argparse.ArgumentParser(description="SLP xy keypoints no-MLP concat sampler")
...
def run_inference(args):
    ...
    # Override target to use the local Identity class
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc="Sampling")):
            ...
            # Get Conditioning
            # Identity passes [Batch, 26] -> expands to [Batch, 26, 32, 32]
