...
class SLPPoseBase(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # Although CrossAttn takes a vector, we reshape to 3D for consistency 
        # with LDM collate_fn if necessary.
...
def get_parser():
    parser = argparse.ArgumentParser(description="SLP full keypoints MLP-crossattn sampler")
...
def run_inference(args):
    ...
    # --- CRITICAL CHANGE: Use PoseEmbedder ---
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc="Sampling")):
            ...
            # # Save Skeleton (Uncomment if needed)
