...
class PoseEmbedder(nn.Module):
    # NOTE: Default input_dim is 26 (XY only)
    def __init__(self, input_dim=26, embed_dim=128):
        super().__init__()
        # MLP to "learn" the pose representation from XY coordinates
    ...
    def forward(self, x):
        ...
        # 1. Handle dimensions: Flatten to (Batch, 26)
        x = x.view(x.size(0), -1) 
...
class SLPPoseBase(Dataset):
    ...
    def get_raw_keypoints(self, txt_path):
        ...
        # Parse triplets (x, y, v) but only keep x, y
    ...
    def __getitem__(self, i):
        ...
        # Reshape to (Batch, 1, 1, 26) for DataLoader consistency
...
def save_pose_visualization(pose_tensor, out_path, height, width):
    """
    Visualizes 26-channel (XY only) pose.
    """
...
def get_parser():
    parser = argparse.ArgumentParser(description="SLP xy keypoints MLP-concat sampler")
...
def run_inference(args):
    ...
    # --- Override target to use the local PoseEmbedder ---
    print(f"Overriding conditioner target to use local PoseEmbedder (XY MLP)...")
    config.model.params.cond_stage_config.target = "__main__.PoseEmbedder"
    
    # Ensure params match 26 input dims just in case config is ambiguous
    # (Though usually it reads from YAML params)
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc="Sampling")):
            ...
            # --- Get Conditioning ---
            # Passes 26 values -> MLP -> 128 embedding -> Broadcast to 32x32
