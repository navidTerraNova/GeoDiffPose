...
class PoseEmbedder(nn.Module):
    # CHANGED: Input dim is now 26 (13 keypoints * 2 coords)
...
class SLPPoseBase(Dataset):
    ...
    def get_raw_keypoints(self, txt_path):
        ...
        # Assuming format is still x, y, v in the text file
...
def save_pose_visualization(pose_tensor, out_path, height, width):
    # CHANGED: Visualization logic for XY only (no visibility flag)
    data = pose_tensor.cpu().numpy().flatten()
    canvas = np.zeros((height, width, 3), dtype=np.uint8)
    
    points = []
    # Loop with step 2 for X, Y
    ...
    # Standard COCO-style limbs (indices must match your keypoint order)
    limbs = [
        (1, 2), (1, 7), (2, 8), (7, 8),   
        (1, 3), (3, 5),                   
        (2, 4), (4, 6),                   
    ...
    for p1, p2 in limbs:
        if p1 < len(points) and p2 < len(points):
            pt_a = points[p1]
            pt_b = points[p2]
            # Draw line (we assume all points are visible in XY mode)
            cv2.line(canvas, pt_a, pt_b, color_limb, 2)

    for pt in points:
        cv2.circle(canvas, pt, 3, color_joint, -1)
...
def get_parser():
    parser = argparse.ArgumentParser(description="SLP xy keypoints MLP-crossattn sampler")
...
def run_inference(args):
    ...
    # --- CRITICAL CHANGE: Use local PoseEmbedder ---
    print(f"Overriding conditioner target to use local PoseEmbedder (XY)...")
    config.model.params.cond_stage_config.target = "__main__.PoseEmbedder"
    
    # Ensure params match the XY setup (26)
    config.model.params.cond_stage_config.params.input_dim = 26
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc="Sampling")):
            ...
            # Optional: Save visual skeleton to verify coordinates
            ...
            # Get Conditioning
            # The PoseEmbedder processes this into [Batch, 1, 512]
